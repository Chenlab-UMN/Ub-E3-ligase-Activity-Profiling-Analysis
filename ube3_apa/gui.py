...
if __name__ == '__main__':
    ...
    def getInput():
        ...
        inputFile = filedialog.askopenfile(mode='r', filetypes=[('CSV Files', '*.csv')])
    ...
    lblOutput = tk.Label(window, text="Experiment Label for Export Files")
    ...
    def run(inputDirectory, inputType, outputDirectory, exp_label, pronorm, inputDirectory_pro, grouped, ratio, log2trans, t, b):
        ...
        def threadWorker(inputDirectory, inputType, outputDirectory, exp_label, pronorm, inputDirectory_pro, grouped, ratio, log2trans, b):
            ...
            if(boolify(pronorm) == True):
                ube3_apa.e3enrich(pathlib.Path(inputDirectory),inputType,pathlib.Path(outputDirectory),exp_label=exp_label,proratio_dir=pathlib.Path(inputDirectory_pro),grouped=boolify(grouped),ratio_output=boolify(ratio),log2trans=boolify(log2trans))
            else:
                ube3_apa.e3enrich(pathlib.Path(inputDirectory),inputType,pathlib.Path(outputDirectory),exp_label=exp_label,proratio_dir='None',grouped=boolify(grouped),ratio_output=boolify(ratio),log2trans=boolify(log2trans))
    ...
    btnSubmit = tk.Button(window, text="Submit", command=lambda:[switchState(btnSubmit), run(txtInput.get(), inputTypeComboBox.get(), txtOutput.get(), txtExplabel.get(), pronorm, txtInput_pro.get(), grouped, outputRatio, outputLog2Trans, consoleOutput, btnSubmit)])
